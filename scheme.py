...
class Door:
    ...
    def set_point (self, point : Optional[Point]):
        if self.rigid:
            raise RuntimeError('Can not change rigid door point in door ' + str(self))
        self._point = point
        self._segment = None
        self._margined_segment = None
...
class Room:
    ...
    def set_discarded_grid (self, new_discarded_grid):
        # If nothing was changed then stop here, do not reset anything
        if self._discarded_grid == new_discarded_grid:
            return
    ...
    # Update discarded grid
    # This function should be called any time a rigid child grid or the corridor grid are changed
    def update_discarded_grid (self):
    ...
    def get_min_size (self) -> number:
        ...
        if self.parent_building:
            ...
            return self._min_size
        raise InputError('Cannot find minimum size for room ' + self.name + '. Please, set the minimum size in the root room or the buildings room args')
                ...
                if not door.room.rigid:
                    ...
                    if len(available_segments) > 0:
                        ...
                        def expand_room (available_segment : Segment) -> bool:
                            ...
                            if not door.room.expand_grid(expansion_grid, compensate_invaded=False):
                                return False
                    ...
                    # This point will be a corner for both the corridor and the door room and it must always be there
                        ...
                        for corridor_reference_segment in corridor_reference_segments:
                            ...
                            def expand_room (expansion_segment : Segment) -> bool:
                                ...
                                # If we can not expand the grid at this point then we must stop
                                # It may happen, for instance, when we try to expand over a door which can not be relocated anywhere else
                                if not door.room.expand_grid(expansion_grid):
                                    return False
    ...
    def reduce_children_corners (self, verbose : bool = False):
        ...
        for child in self.children:
            ...
            while len(exterior_polygon.corners) > 4:
                ...
                succeed = False
                for zigzag in zigzags:
                    if verbose: print(f'Reducing corners in {zigzag["middle_segment"]}')
    ...
    def fit_to_required_area (self, restricted_segments : list = [], conformist : bool = False, verbose : bool = False) -> bool:
        ...
        # If this is not a conformist call then make sure there is free space available to claim
        if not conformist and not self.free_grid:
            raise RuntimeError(f'Trying to fit {self.name} (not conformist) but there is no free space available')
    ...
    def invade (self, region : Grid, force : bool = False, skip_update_display : bool = False, verbose : bool = False) -> bool:
        ...
        if not self.boundary:
            if verbose: print(' No boundary, no problem')
            return True
        ...
        if not self.truncate(region, force, skip_update_display=skip_update_display):
            ...
            self.boundary = backup_boundary
            return False
        if verbose: print(f'Invading {self.name}, truncated')
        ...
        if not self.fit_to_required_area():
            ...
            self.boundary = backup_boundary
            return False
        if verbose: print(f'Invading {self.name}, succeeded')
        return True
    ...
    def expand_grid (self, expansion_grid : Grid, check_overlaps : bool = True, compensate_invaded : bool = True, verbose : bool = False) -> bool:
        ...
        if self.parent:
            ...
            if check_overlaps:
                ...
                if self.parent.corridor_grid and self.parent.corridor_grid.get_overlap_grid(expansion_grid):
                    ...
                    return False
                # Set log label
                label = 'Invading' if compensate_invaded else 'Truncating'
    ...
    def update_display (self, extra : list = [], title : Optional[str] = None):
        global display_frames_limit
        if not display_solving_process:
            return
        if display_frames_limit:
            display_frames_limit -= 1
            if display_frames_limit == 0:
...
class Building:
    def __init__ (self,
        ...
        if room_args.get('corridor_size', None) == None:
            ...
            self.room_args['corridor_size'] = overall_min_size * 0.9
