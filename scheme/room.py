...
from utils.pyramid import paralelizer
...
class Room:
    ...
    # The parent room
    parent = property(get_parent, None, None, "The parent room")
    ...
    def get_child_by_name (self, child_name : str) -> 'Room':
        child = self._children_by_names.get(child_name, None)
        if child is None: raise RuntimeError(f'Room "{self.name}" has no child named "{child_name}"')
        return child

    # Add a single child room
    def add_child (self, child_room : 'Room'):
        if child_room in self.children: raise ValueError(f'Room {child_room.name} is already a child')
        child_room._parent = self
        self._children.append(child_room)
        if child_room.grid:
    ...
    def get_boundary (self) -> Boundary:
        ...
        if self.grid:
            ...
            if len(boundaries) > 1:
                # Show only the 2 first boundaries
                boundary_1 = boundaries[0]
                boundary_1_segments = [ segment.get_colored_segment('red') for segment in boundary_1.segments ]
                boundary_2 = boundaries[1]
                boundary_2_segments = [ segment.get_colored_segment('blue') for segment in boundary_2.segments ]
                add_frame(boundary_1_segments + boundary_2_segments, 'DEBUG: Showing the first 2 boundaries.')
                raise ValueError(f'A single room must not have more than 1 boundary but room "{self.name}" has {len(boundaries)}')
    ...
    def get_target_area (self) -> number:
        ...
        if not self.parent:
            raise RuntimeError(f'Can not calculate target area since parent room is not defined in room {self.name}')
        self.parent.set_children_target_areas()
        if self._target_area == None: raise ValueError(f'Target area not set in {self.name}')
        ...
        if self.min_size and self._target_area and self._target_area < self.min_size**2:
            print(f'Target area {self.target_area} is not reachable with a minimum size of {self.min_size}')
            raise InputError(f'Room area is not sufficient for the minimum size in room {self.name}')
        return self._target_area
    ...
    def get_donable_area_proportion (self) -> number:
        ...
        if equal(area_range, 0): return 0
    ...
    def set_children_target_areas (self, verbose : bool = False):
        if len(self.children) == 0: raise ValueError(f'Room {self.name} has no children')
        # We assign target areas for non rigid rooms only
        non_rigid_children = [ child for child in self.children if not child.rigid ]
        if verbose: print(f'Setting {self.name} children target areas: ' + ', '.join([ child.name for child in non_rigid_children ]))
    ...
    # Get the required outside space for rigid doors and make it part of the parent corridor already
    # DANI: No se ha provado
    def reserve_doors_self_required_space (self):
        for door in self._doors:
            if not door.rigid: continue
            expanded_corridor_gird = self.corridor_grid + Grid([door.get_required_space(inside=True)])
            self.set_corridor_grid(expanded_corridor_gird, skip_update_display = True)

    # Get the required inside space for rigid doors and make it part of the self corridor already
    def reserve_doors_parent_required_space (self):
        if not self.parent: return
        for door in self._doors:
            if not door.rigid: continue
            expanded_corridor_gird = self.corridor_grid + Grid([door.get_required_space(inside=False)])
            self.parent.set_corridor_grid(expanded_corridor_gird, skip_update_display = True)
    ...
    def solve_children (self, recursive : bool = False, verbose : bool = True) -> bool:
        if verbose: print(f'Solving {self.name} children: ' + ', '.join([ child.name for child in self.children ]))
        # Make a list of the rooms to be solved
        # These are children which have no boundary already
        rooms_to_solve = [ child for child in self.children if not child.boundary ]
        # If there are not children then we have nothing to do here
        if len(rooms_to_solve) == 0: return True
        ...
        if self._child_adaptable_boundary:
            random.shuffle(rooms_to_solve)
        else:
            def sort_by_target_area (room):
    ...
    # Set a function to try to stablish a child grid from a raw starting spot
    def _try_initial_grid (self : 'Room', child : 'Room',
        target_parent_grid : 'Grid', initial_child_grid : 'Grid',
        tried_fitted_initial_grids : set, verbose : bool = False,
    ) -> bool:
        ...
        fitted_initial_child_grid = target_parent_grid.force_fit(
            initial_child_grid, self.min_size, child.min_size, expand=True)
        # If we failed to fit the grid then try the next
        if type(fitted_initial_child_grid) is Exception: return False
        # If we already tried this initial grid and further failed then skip it already
        if fitted_initial_child_grid in tried_fitted_initial_grids: return False
        tried_fitted_initial_grids.add(fitted_initial_child_grid)
        # Now claim the actual initial grid
        if not child.expand_grid(fitted_initial_child_grid, check_parent_free_grid=False):
            return False
    ...
    def set_corridor (self, verbose : bool = False) -> bool:
        ...
        def make_corridor_grid (debug : bool = True):
            ...
            while corridor_grid and not corridor_grid.is_unified():
                ...
                raise RuntimeError(f'We have a splitted corridor in room {self.name}')
            
            # If there is no corridor grid at this point then it may the defaultl placed corridor already convers all rooms
            if not corridor_grid: return
    ...
    def fit_to_required_area (self,
        ...
        while not self.is_fit_to_required_area(behaviour=behaviour):
            ...
            self.parent.restore_grid_backup(backup, title='Restored grid backup while fitting to required area')
            return False
        return True
    ...
    def expand_frontier (self,
        ...
        def push_segment (pushed_segment : Segment, protocol : int = 1, verbose : bool = False) -> bool:
            ...
            if push_length < minimum_resolution:
                if verbose: print(f' The push length is too small for segment {pushed_segment}')
                ...
                if verbose: print(f' Segment has been reduced to {pushed_segment}')
        ...
        problem_a = margin_a and lower(margin_a.length, margin_limit)
        problem_b = margin_b and lower(margin_b.length, margin_limit)
        ...
        for i, problem in enumerate([ problem_a, problem_b ]):
            ...
            if verbose: print(f'Problem with margin {margin}')
            ...
            # In case the new point has passed the other point we stop here
            if new_point not in reduced_frontier:
                if verbose: print(f'Expanding frontier {frontier} failed: the frontier has been fully consumed')
                return False
            # Calculate the distance between the two points
            # WARNING: Note that the distance may be 0 and thus delcaring the segment here would fail
            reduced_size = new_point.get_distance_to(other_point)
            if equal(reduced_size, 0):
                if verbose: print(f'Expanding frontier {frontier} failed: the reduced frontier is non-existent')
                return False
            # If the frontier is smaller than the minimum size then stop here
            if lower(reduced_size, self.min_size):
                if verbose: print(f'Expanding frontier {frontier} failed: the reduced frontier is not wide enought ({reduced_size})')
                return False
    ...
    def contract_frontier (self, frontier : Segment, required_area : number, verbose : bool = False, _recursion_depth : int = 0) -> bool:
        ...
        if len(rects) == 0:
            raise RuntimeError(f'No pot ser: {grid}')
        ...
        for i, problem in enumerate([ problem_a, problem_b ]):
            ...
            if new_point not in reduced_frontier:
                if verbose: print(f'Contracting frontier {frontier}: the frontier has been fully consumed')
                return False
            # If the frontier is smaller than the minimum size then stop here
            frontier_length = new_point.get_distance_to(other_point)
            if lower(frontier_length, self.min_size):
                if verbose: print(f'Contracting frontier {frontier}: the reduced frontier is not wide enought ({frontier_length})')
                return False
            reduced_frontier = Segment(new_point, other_point)
    ...
    # Use the force argument to skip all checkings and simply truncate the grid
    ...
    # Use the skip_update_display to avoid this truncattion to generate a display frame
    ...
    def truncate_grid (self,
        ...
        if not truncated_grid:
            if verbose: print(' Failed to truncate grid: The room would be fully consumed')
            return False
        # If the truncation was forced then remove as much extra regions as needed to keep the minimum margin
        if force:
        ...
        if not force and not self.check_doors_required_spaces(truncated_grid):
            ...
            if not self.relocate_doors(truncated_boundary):
                if verbose: print(' Failed to truncate grid: The room would not respect door spaces')
                return False
        ...
        if not self.grid.check_minimum(self.min_size):
            ...
            if easy:
                ...
                self.restore_grid_backup(backup, title='Restored grid backup while truncating')
                return False
            if verbose: print(' Minimum size conflict when truncating: Grid will be force-fitted')
            if self.parent:
                ...
                if not available_grid.check_minimum(self.parent.min_size):
                    ...
                    self.restore_grid_backup(backup, title='Restored grid backup while truncating')
                    return False
                if verbose: print(f'Checking for room {self.name}, whose parent is {self.parent.name}')
                ...
                self.set_grid(fitted_grid, skip_update_display=skip_update_display)
    ...
    def expand_grid (self,
        ...
        if check_overlaps:
            ...
            # Keep a list of borther rooms we are overlapping with
            ...
            if compensate_invaded:
                for brother_room in invaded_brother_rooms:
    ...
    # Other internal values should be reset normally
    def make_backup (self) -> dict:
        return {
            'grid': self.grid,
            'corridor': self.corridor_grid,
            'doors': { door: door.make_backup() for door in self.doors },
            'children': { child: child.make_backup() for child in self.children },
        }

    # Restor a full backup
    def restore_backup (self, backup : dict, display : bool = True) -> dict:
        # Recover self grid
        backup_grid = backup.get('grid', None)
        if backup_grid is not None:
            self.set_grid(backup_grid, skip_update_display = True)
        # Recover self corridor grid
        backup_corridor_grid = backup.get('corridor', None)
        if backup_corridor_grid is not None:
            self.set_corridor_grid(backup_corridor_grid, skip_update_display = True)
        # Recover doors
        backup_doors = backup.get('doors', None)
        if backup_doors is not None:
            self.doors = [ door for door in backup_doors.keys() ]
            for door, backup_door in backup_doors.items():
                door.restore_backup(backup_door)
        # Recover children
        backup_children = backup.get('children', None)
        if backup_children is not None:
            self._children = [ child for child in backup_children.keys() ]
            for child, backup_child in backup_children.items():
                child.restore_backup(backup_child, display=False)
        # Update the recover
        if display:
            self.update_display(title=f'Recovered backup in room "{self.name}"')
